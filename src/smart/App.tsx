...
type PortalUserLike = { role?: string; full_name?: string; email?: string } | null;

type AppProps = {
  embedded?: boolean;
  portalUser?: PortalUserLike;
};

const resolveEmbeddedRole = (portalUser: PortalUserLike): UserRole => {
  return String(portalUser?.role || '').toLowerCase() === 'client' ? UserRole.CLIENT : UserRole.ADMIN;
};

const resolveEmbeddedAdmin = (portalUser: PortalUserLike) => ({
  name: portalUser?.full_name || portalUser?.email || 'HELM Portal',
  title: String(portalUser?.role || '').toLowerCase() === 'client' ? 'بوابة الموكل' : 'إدارة المكتب',
});

const App: React.FC<AppProps> = ({ embedded = false, portalUser = null }) => {
  const [userRole, setUserRole] = useState<UserRole | null>(() => embedded ? resolveEmbeddedRole(portalUser) : null);
  ...
  // Admin Profile State
  const [adminProfile, setAdminProfile] = useState<{name: string, title: string} | null>(() => embedded ? resolveEmbeddedAdmin(portalUser) : null);
  ...
  // When HELM Smart is embedded inside HELM Portal, it must not show its own login screen.
  useEffect(() => {
    if (!embedded) return;
    const nextRole = resolveEmbeddedRole(portalUser);
    setUserRole(nextRole);
    if (nextRole === UserRole.ADMIN) {
      setLoggedInClient(null);
      setAdminProfile(resolveEmbeddedAdmin(portalUser));
    }
  }, [embedded, portalUser?.role, portalUser?.full_name, portalUser?.email]);
