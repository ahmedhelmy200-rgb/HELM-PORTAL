...
fn with_connection<T>(state: State<'_, AppState>, work: impl FnOnce(&Connection) -> Result<T, String>) -> Result<T, String> {
...
fn cache_upsert_many(
    ...
    for row in rows {
        let Some(record_id) = row.get("id").and_then(Value::as_str) else {
            continue;
        };
        transaction
            .execute(
...
#[tauri::command(rename_all = "camelCase")]
fn cache_get(state: State<'_, AppState>, entity: String, record_id: String) -> Result<Option<Value>, String> {
    with_connection(state, |connection| {
...
fn cache_list(state: State<'_, AppState>, entity: String) -> Result<Vec<Value>, String> {
    with_connection(state, |connection| {
        let mut statement = connection
            .prepare("SELECT data_json FROM records WHERE entity = ?1 AND deleted = 0 ORDER BY updated_at DESC")
            .map_err(|error| error.to_string())?;
...
#[tauri::command(rename_all = "camelCase")]
fn cache_delete(state: State<'_, AppState>, entity: String, record_id: String) -> Result<(), String> {
    with_connection(state, |connection| {
        connection
            .execute(
...
#[tauri::command(rename_all = "camelCase")]
fn cache_mark_clean(state: State<'_, AppState>, entity: String, record_id: String) -> Result<(), String> {
    with_connection(state, |connection| {
        connection
            .execute(
    ...
    with_connection(state, |connection| {
        connection
            .execute(
                ...
                VALUES (?1, ?2, ?3, ?4, ?5)
                "#,
                params![operation, entity, record_id, payload.to_string(), Utc::now().to_rfc3339()],
            )
            .map_err(|error| error.to_string())?;
...
#[tauri::command]
fn outbox_list(state: State<'_, AppState>, limit: Option<i64>) -> Result<Vec<OutboxEntry>, String> {
    with_connection(state, |connection| {
        let mut statement = connection
            .prepare(
                r#"
...
#[tauri::command(rename_all = "camelCase")]
fn outbox_mark_failed(state: State<'_, AppState>, id: i64, error: String) -> Result<(), String> {
    with_connection(state, |connection| {
        connection
            .execute(
...
#[tauri::command(rename_all = "camelCase")]
fn meta_set(state: State<'_, AppState>, key: String, value: String) -> Result<(), String> {
    with_connection(state, |connection| {
        connection
            .execute(
                r#"
