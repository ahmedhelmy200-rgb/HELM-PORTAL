...
export default function BadayatAlKhair() {
  ...
  return (
    <div dir="rtl" className="space-y-6 text-slate-100">
      <section className="relative overflow-hidden rounded-[2rem] border border-amber-300/20 bg-slate-950/75 p-6 shadow-2xl">
        <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_15%_15%,#f59e0b,transparent_34%),radial-gradient(circle_at_85%_10%,#38bdf8,transparent_30%)]" />
        <div className="relative flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-sm font-black tracking-[.2em] text-amber-300">BADAYAT AL KHAIR</p>
            <h1 className="mt-2 text-3xl font-black md:text-4xl">قسم بداية الخير</h1>
            <p className="mt-2 max-w-4xl leading-8 text-slate-300">لوحة موحدة ومنظمة لإدارة الموظفين والفروع والإقامات والعقود والحسابات والجزاءات والمكافآت والمهام والنماذج الرسمية، بدون جزء الصلاحيات الداخلي.</p>
            <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
              ...
              <span className="rounded-full bg-white/10 px-3 py-1 text-slate-300">إجمالي الموظفين: {employees.length}</span>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setActiveBranch("templates")} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-4 py-3 font-black text-white hover:bg-white/15"><FileText className="h-4 w-4" /> النماذج</button>
            <button onClick={addEmployee} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-amber-400 px-5 py-3 font-black text-slate-950 shadow-lg hover:bg-amber-300"><Plus className="h-4 w-4" /> إضافة موظف</button>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 gap-3 md:grid-cols-5">
        {branches.map((branch) => (
          <button key={branch.id} onClick={() => setActiveBranch(branch.id)} className={`rounded-3xl border p-4 text-right transition ${activeBranch === branch.id ? "border-amber-300 bg-white/12 shadow-xl" : "border-white/10 bg-white/5 hover:bg-white/10"}`}>
            <div className={`mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${branch.color}`}><Building2 className="h-6 w-6" /></div>
            <h3 className="text-sm font-black leading-6">{branch.name}</h3>
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <Stat icon={ShieldCheck} label="إقامات خلال 60 يوم" value={stats.expiring} danger={stats.expiring > 0} />
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-[360px_1fr]">
          <aside className="rounded-3xl border border-white/10 bg-slate-950/65 p-4">
            ...
            <div className="mb-4 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="بحث عن موظف..." className="w-full bg-transparent text-sm outline-none" />
            </div>
                ...
                <button key={emp.id} onClick={() => setSelectedId(emp.id)} className={`w-full rounded-2xl border p-3 text-right ${selectedEmployee?.id === emp.id ? "border-amber-300 bg-amber-300/10" : "border-white/10 bg-white/5 hover:bg-white/10"}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-slate-400">{emp.jobTitle || "بدون مسمى"} · {emp.residencyStatus || "غير محدد"}</p>
                    </div>
                    {daysLeft(emp.residencyExpiry) !== null && daysLeft(emp.residencyExpiry) <= 60 && <span className="rounded-full bg-red-500/15 px-2 py-1 text-[10px] font-black text-red-200">متابعة</span>}
                  </div>
                </button>
              ))}
              {!branchEmployees.length && <p className="py-10 text-center text-sm text-slate-400">لا توجد بيانات موظفين داخل هذا الفرع.</p>}
            </div>
          </aside>
...
function Stat({ icon: Icon, label, value, danger }) {
  return <div className={`rounded-3xl border p-4 ${danger ? "border-red-400/40 bg-red-500/10" : "border-white/10 bg-white/5"}`}><Icon className="mb-3 h-5 w-5 text-amber-300" /><p className="text-2xl font-black">{value}</p><p className="mt-1 text-xs text-slate-400">{label}</p></div>;
}

function Avatar({ emp }) {
  return <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br from-amber-400 to-slate-800">{emp.photo ? <img src={emp.photo} alt={emp.fullName} className="h-full w-full object-cover" /> : <Users className="h-6 w-6 text-white" />}</div>;
}
...
function Panel({ title, icon: Icon, children, className = "" }) {
  return <section className={`rounded-3xl border border-white/10 bg-slate-950/65 p-5 ${className}`}><h3 className="mb-4 flex items-center gap-2 text-lg font-black"><Icon className="h-5 w-5 text-amber-300" />{title}</h3>{children}</section>;
}

function ListPanel({ title, items, empty, icon = ClipboardCheck }) {
  const Icon = icon;
  return <Panel title={title} icon={Icon}>{items?.length ? <div className="space-y-2">{items.map((it, i) => <div key={i} className="rounded-2xl bg-white/5 p-3"><b>{it.title}</b><p className="mt-1 text-xs text-slate-400">{it.date} · {money(it.amount)}</p>{it.note && <p className="mt-1 text-sm text-slate-300">{it.note}</p>}</div>)}</div> : <p className="text-slate-400">{empty}</p>}</Panel>;
}
  ...
  return <div className="space-y-4">
    <div className="rounded-3xl border border-white/10 bg-slate-950/65 p-5">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4"><Avatar emp={emp} /><div><p className="text-xs font-black text-amber-300">{activeBranchInfo.name}</p><h2 className="text-2xl font-black">{emp.fullName}</h2><p className="text-slate-400">{emp.jobTitle || "بدون مسمى"} · {emp.department || "بدون قسم"}</p></div></div>
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      <Panel title="بيانات الموظف" icon={Users}>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <label className="block"><span className="text-xs font-bold text-slate-400">صورة الموظف من الجهاز</span><input type="file" accept="image/*" onChange={(e) => handlePhotoUpload(e.target.files?.[0])} className="mt-1 w-full rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-sm" /></label>
        </div>
      </Panel>

      <Panel title="العقد والإقامة" icon={ShieldCheck}>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
      ...
      <Panel title="الشؤون القانونية والإشعارات" icon={BriefcaseBusiness}>
        ...
        <textarea value={legalNote} onChange={(e) => setLegalNote(e.target.value)} placeholder="إشعار / ملاحظة قانونية للموظف" className="min-h-[92px] w-full rounded-2xl border border-white/10 bg-white/5 p-3 outline-none" />
        <div className="mt-2 flex flex-wrap gap-2"><button onClick={() => notifyEmployee("audit")} className="inline-flex items-center gap-2 rounded-2xl bg-blue-500 px-4 py-2 font-black"><Bell className="h-4 w-4" /> تسجيل إشعار</button><button onClick={() => notifyEmployee("whatsapp")} className="inline-flex items-center gap-2 rounded-2xl bg-emerald-500 px-4 py-2 font-black"><MessageCircle className="h-4 w-4" /> واتساب</button><button onClick={() => notifyEmployee("email")} className="inline-flex items-center gap-2 rounded-2xl bg-sky-500 px-4 py-2 font-black"><Mail className="h-4 w-4" /> إيميل</button></div>
      </Panel>
    </div>
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      ...
      <Panel title="الطباعة والمهام الخاصة" icon={Printer}>
        <div className="space-y-2">
          <button onClick={() => window.print()} className="w-full rounded-2xl bg-white/10 px-4 py-3 font-black hover:bg-white/15">طباعة ملف الموظف</button>
          <div className="space-y-2 rounded-2xl bg-white/5 p-3"><input value={taskForm.title} onChange={(e) => setTaskForm({ ...taskForm, title: e.target.value })} placeholder="عنوان المهمة الخاصة" className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 outline-none" /><input type="datetime-local" value={taskForm.due_date} onChange={(e) => setTaskForm({ ...taskForm, due_date: e.target.value })} className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 outline-none" /><select value={taskForm.priority} onChange={(e) => setTaskForm({ ...taskForm, priority: e.target.value })} className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2"><option>عالية</option><option>متوسطة</option><option>منخفضة</option></select><button onClick={createSpecialTask} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-amber-400 px-4 py-3 font-black text-slate-950"><CheckSquare className="h-4 w-4" /> إنشاء مهمة خاصة</button></div>
        </div>
      </Panel>
    </div>
  </div>;
...
function AuditPanel({ audit = [] }) {
  return <Panel title="سجل الحركة داخل قسم بداية الخير" icon={CalendarClock}><div className="max-h-72 space-y-2 overflow-y-auto">{audit.length ? audit.slice(0, 40).map((item, i) => <div key={i} className="rounded-2xl border border-white/10 bg-white/5 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><b>{item.action}</b><span className="text-xs text-slate-400">{item.at}</span></div><p className="mt-1 text-sm text-slate-300">{item.employee}</p>{item.note && <p className="mt-1 text-xs leading-6 text-slate-400">{String(item.note).slice(0, 260)}</p>}</div>) : <p className="text-slate-400">لا يوجد سجل حركة بعد.</p>}</div></Panel>;
}

function EmptyEmployee() {
  return <div className="rounded-3xl border border-white/10 bg-slate-950/65 p-8 text-center"><Users className="mx-auto mb-3 h-10 w-10 text-amber-300" /><h3 className="font-black">لا يوجد موظف محدد</h3><p className="mt-2 text-sm text-slate-400">أضف موظفاً جديداً أو اختر فرعاً يحتوي على موظفين.</p></div>;
}
...
function TemplatesPanel({ employees, selectedEmployee }) {
  ...
  return <section className="grid grid-cols-1 gap-4 xl:grid-cols-[360px_1fr]">
    <aside className="rounded-3xl border border-white/10 bg-slate-950/65 p-4">
      <h2 className="mb-1 text-xl font-black">النماذج الشاملة</h2>
      <p className="mb-4 text-sm leading-7 text-slate-400">اختر النموذج والموظف، عدّل النص، ثم اطبع أو صدّر PDF مباشرة.</p>
      ...
      <div className="space-y-3">{templateGroups.map((group) => <div key={group.group} className="rounded-2xl border border-white/10 bg-white/5 p-3"><h3 className="mb-2 text-sm font-black text-amber-200">{group.group}</h3><div className="space-y-1">{group.items.map((item) => <button key={item} onClick={() => setTemplateName(item)} className={`w-full rounded-xl px-3 py-2 text-right text-sm ${templateName === item ? "bg-amber-400 text-slate-950 font-black" : "hover:bg-white/10"}`}>{item}</button>)}</div></div>)}</div>
    </aside>
    <main className="space-y-4">
      <Panel title={`تحرير النموذج: ${templateName}`} icon={FileText}>
        <textarea value={body} onChange={(e) => setBody(e.target.value)} className="min-h-[270px] w-full rounded-2xl border border-white/10 bg-white/5 p-4 leading-8 outline-none focus:border-amber-300" />
        <div className="mt-3 flex flex-wrap gap-2"><button onClick={() => saveBadayatTemplateDraft(templateName, body)} className="inline-flex items-center gap-2 rounded-2xl bg-emerald-500 px-4 py-2 font-black"><Save className="h-4 w-4" /> حفظ مسودة النموذج</button><button onClick={() => { resetBadayatTemplateDraft(templateName); setBody(getBadayatTemplateBody(templateName)); }} className="inline-flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-2 font-black"><RotateCcw className="h-4 w-4" /> استعادة الأصلي</button><button onClick={() => printBadayatDocument(templateName, rendered)} className="inline-flex items-center gap-2 rounded-2xl bg-blue-500 px-4 py-2 font-black"><Printer className="h-4 w-4" /> طباعة</button><button onClick={() => exportBadayatDocumentPdf(templateName, rendered)} className="inline-flex items-center gap-2 rounded-2xl bg-amber-400 px-4 py-2 font-black text-slate-950"><Download className="h-4 w-4" /> PDF مباشر</button></div>
      </Panel>
      <Panel title="معاينة نهائية" icon={Sparkles}><div className="rounded-2xl border border-white/10 bg-white p-5 text-right text-slate-950"><h2 className="mb-4 text-center text-2xl font-black">{templateName}</h2><pre className="whitespace-pre-wrap font-[inherit] leading-9">{rendered}</pre></div></Panel>
